...
import { addObjednavku } from '../../services/objednavky';
import { Objednavka } from '../../services/types';

export default function PridanieObjednavkyForm({ onSubmit }: { onSubmit: () => void }) {

    const handleSubmit = (values: FormikValues) => {
        const output: Omit<Objednavka, 'id'> = {
            sirka: values.hrubka,
            dlzka: values.dlzka,
            pocet: values.pocet,
            sum: values.pocet * values.dlzka
        }
        addObjednavku(output).then(onSubmit)
